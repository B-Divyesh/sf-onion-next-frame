...
const revalidatedAssetCache = 'public, max-age=0, must-revalidate';
const revalidatedAssetPaths = new Set([
    ...
    configurePreviewServer(server) {
      server.middlewares.use((request, response, next) => {
        ...
        if (url.pathname.startsWith('/assets/')) {
          response.setHeader('Cache-Control', revalidatedAssetPaths.has(url.pathname) ? revalidatedAssetCache : immutableAssetCache);
        }
